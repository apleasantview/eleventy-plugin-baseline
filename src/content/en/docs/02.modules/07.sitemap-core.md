...
- If enabled, registers a virtual template at `/sitemap.xml` that loops over `collections.all`, filters out excluded items, and emits a `<urlset>` with `loc`, optional `lastmod`, optional `changefreq`, and optional `priority`.
