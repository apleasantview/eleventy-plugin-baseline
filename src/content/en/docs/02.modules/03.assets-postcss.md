...
### Inlining your CSS
- Any CSS file can be inlined with `inlinePostCSS`; the filter processes the exact file you pass through PostCSS.
- In a template or Markdown file, build the path (e.g., from `_baseline.assets.assetsInput`) and pipe it through `inlinePostCSS`.
- In Markdown, keep the call unindented and on its own lines so the `<style>` is treated as raw HTML.

Example:
```njk
{% raw %}{% set cssPath = _baseline.assets.assetsInput ~ "css/critical.css" %}{% endraw %}
{% raw %}{{ cssPath | inlinePostCSS | safe}}{% endraw %}
```
