...
- `siteUrl` (string): optional absolute site URL; falls back to `site.url` or `process.env.URL`/`DEPLOY_URL`/`DEPLOY_PRIME_URL`.
- `pathPrefix` (string): optional path prefix override; defaults to Eleventy `pathPrefix`.
...
- Canonical prefers `head.canonical`, then `page.url`/content map, and is absolutized with `site.url`/`siteUrl`/env URL + `pathPrefix`.
