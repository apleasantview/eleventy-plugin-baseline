...
## 2) Add a debug page
Create `src/content/pages/debug.md`:
```md
---
...
## Collections (first 3)
<pre>
{% raw %}{% for item in collections.all | slice(0, 3) %}
- {{ item.url }}
{% endfor %}{% endraw %}
</pre>

## Page keys
<pre>{% raw %}{{ page | _keys }}{% endraw %}</pre>

## Page data (inspect)
<pre>{% raw %}{{ page | _inspect({ getters: true, depth: 2 }) }}{% endraw %}</pre>

## Page data (json)
<pre>{% raw %}{{ page | _json(2) }}{% endraw %}</pre>
```

## 3) Use navigator helpers inline
If you want to see the full data/context on this page:
```njk
{% raw %}<h2>Navigator snapshot (inline)</h2>
{{ _navigator() }}{% endraw %}
```
`_navigator()` renders the full snapshot; `_context()` renders just the current context. Use only in local dev.
This dumps a large snapshot; use only in local dev.

## 4) Visit the navigator template (optional)
With `enableNavigatorTemplate: true`, Baseline exposes a virtual page at `/navigator-core.html`. Disable this option before production.

## 5) Run and inspect
```bash
npm run dev
```
- Open `/debug/` to see filter output.
