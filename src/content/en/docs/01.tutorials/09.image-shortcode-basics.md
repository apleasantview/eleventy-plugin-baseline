...
- Figure/attrs: set `caption` (or `figure: false`) to control the figure wrapper; `attrs.class` merges with `imageClass`
...
- Common options (cheat sheet):
  ...
  - `loading` (`"lazy"`/`"eager"`), `containerClass`, `imageClass`, `figure`, `setDimensions`
  - `outputDir`, `urlPath`, `attrs` (merges `class` with `imageClass`)
