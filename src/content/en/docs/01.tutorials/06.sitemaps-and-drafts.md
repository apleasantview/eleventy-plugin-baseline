...
To exclude a page from the sitemap (but still render it):
```md
sitemap:
  ignore: true
```
...
- Per-page: `noindex: true` in front matter is not used by Baseline. To exclude a page, choose one (or combine):
...
```
- Open pages normally.
- Check `dist/sitemap.xml` while dev runs (sitemap is emitted).
- Draft pages still render in dev but won’t appear in the sitemap when you build with `ELEVENTY_RUN_MODE=build`.
