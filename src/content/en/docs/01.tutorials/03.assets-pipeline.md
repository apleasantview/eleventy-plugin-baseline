...
## 5) Add a small inline example (optional)
If you want to inline a tiny CSS or JS snippet into a page, use the filters Baseline registers:
```njk
{% raw %}<style>
  {{ "main { border-top: 2px solid #0ea5e9; }" | inlinePostCSS | safe }}
</style>
<script>
  {{ "console.log('Inline JS via esbuild');" | inlineESbuild | safe }}
</script>{% endraw %}
```
