...
2. Add Baseline to `eleventy.config.js`.
   ...
   Add a local `.env` for development so `process.env.URL` is set:
...
3. Move Eleventy folders under `src/`.
   ...
   Ensure your layouts and includes resolve from `src/_includes`, and content lives under `src/content/...`. Delete the `sitemap.xml.njk` template, now residing in `src/content/`.

4. Add required data files.

   `src/_data/site.js` (use your prod URL in CI; localhost is fine in dev):
   ```js
   export default {
...
6. Add `<baseline-head>` to `base.njk`.
   ...
   Insert `<baseline-head>` in `<head>`, keep the feed link and zachleat anchors module:
...
## Next steps
- Organize content by moving standalone pages into a `src/content/pages/` folder and set explicit permalinks for each.
