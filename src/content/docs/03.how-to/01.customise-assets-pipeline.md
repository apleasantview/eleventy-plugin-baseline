...
For one-offs, pass options directly to the filter. They merge over the processor's own defaults (`minify: true`, `target: 'es2020'`), which is what the filter starts from whatever you set on `assets.esbuild`:
