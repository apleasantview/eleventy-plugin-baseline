...
Keys are dot-paths, so `nav.about` reads the nested key. The language comes from the page, falling back to your `defaultLanguage`.
...
- **Nunjucks specifically.** The filter reads the cascade the way Nunjucks exposes it, which is what Baseline sets for both HTML and markdown. Add another template engine and `t` returns the key there rather than the string.

Full behaviour, including the lookup order, is in the [[filters | filters reference]].
