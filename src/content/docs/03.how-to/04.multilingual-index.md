...
- Guard `item.url` inside the loop. `collections.all` also carries records with `permalink: false`, which arrive with `url: false` and would otherwise render as `href="false"`. The same check is where you filter drafts or `noindex` out (e.g. {% raw %}`{% if item.url and not item.data.draft and not item.data.noindex %}`{% endraw %}).
