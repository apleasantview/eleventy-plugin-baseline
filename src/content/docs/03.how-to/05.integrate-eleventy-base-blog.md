...
- `settings.url` will be set via env (`BASE_URL`) for production.
...
- Set `base` to `process.env.BASE_URL`.
...
In CI and production, set `BASE_URL` to your live origin (e.g. `https://www.example.com`); Baseline uses it for canonical URLs and the sitemap.
