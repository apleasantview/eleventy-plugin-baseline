...
The homepage is in neither folder, so neither file reaches it. It takes the key directly:
...
Same reasoning as the permalink rule, and the same shape: the folder decides for the files inside it, and a page outside, or one that disagrees, says so in its own front matter. Set it once, forget it, break the rule where you need to.
