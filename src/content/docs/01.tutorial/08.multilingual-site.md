...
Change `BASE_URL` in `.env` to an absolute URL. On a deployed site that's your real production URL; for this exercise, any absolute URL (e.g. `https://www.example.com/`) works to verify the output. Then run:
...
- Reset `BASE_URL` in `.env` to "http://localhost:8080/".
