...
Create a local `.env` so canonical URLs, hreflang, and sitemaps use a real origin during development. In production, set `BASELINE_URL` in your hosting environment so the same outputs reach for the live domain instead.
