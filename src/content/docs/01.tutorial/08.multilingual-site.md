...
The site you built over the previous six pages: Node 22 or newer, `"type": "module"`, and the `start` and `build` scripts already in `package.json`.
...
Create a local `.env` so canonical URLs, hreflang, and sitemaps use a real origin during development. In production, set `BASE_URL` in your hosting environment so the same outputs reach for the live domain instead.
...
Give the build a real origin, the same way you did on page six:
