...
- View source on a rendered page. The `<link rel="canonical">` and every absolute link in the head should use the right origin. If `URL` is set, none of them should say `localhost`.
...
- Pair with the [head and hreflang tutorial](/docs/tutorial/head-and-hreflang/) to confirm hreflang URLs use the correct origin once multilingual is on.
