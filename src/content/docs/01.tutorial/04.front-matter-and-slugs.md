...
It never has to catch the data file itself, though. Baseline registers `.js` as a template format so it can compile the JavaScript in `assets/`, which would otherwise put `pages.11tydata.js` in line to be rendered as a page. Baseline excludes `*.11tydata.js` from the build for you, everywhere in the project.
