...
Then the same two lines, in the same place, in the posts file:
...
Posts are in a different folder and directory data does not reach across. Two folders, two copies. That is the cost of the rule being per-folder, and it is the cost you would pay anyway the first time you wanted posts to look different from pages.
...
One thing is missing from the shape: the homepage lists posts but says nothing about the site. Open `src/content/pages/index.md` and add this above the `## Latest posts` heading, so the site introduces itself before it lists anything:
