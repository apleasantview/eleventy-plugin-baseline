...
`dist/sitemap.xml`, which nobody asked for. The sitemap module is on by default and lists the pages you wrote, and the URLs in it start with the origin you passed rather than `localhost`. If they say localhost, `BASELINE_URL` did not reach the build.
