...
Before building, give yourself something that should not survive the build. Create `src/content/posts/1991-08-06-not-finished.md`:
...
With the server running, the homepage lists two posts and `/posts/not-finished/` opens. Leave it there. The build below is where it disappears.
...
`dist/posts/not-finished/`, which is not there. The draft you wrote a minute ago never reached the build, and the sitemap above lists three URLs rather than four for the same reason.
