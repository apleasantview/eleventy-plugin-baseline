...
`title` and `description` are quietly doing more than they look. Both feed the head Baseline assembles, though none of that shows up yet: there is no head to put them in until you add `<baseline-head>` two pages from now.
