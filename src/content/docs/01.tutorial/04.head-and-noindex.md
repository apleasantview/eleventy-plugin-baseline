...
Get a clean, correct `<head>` on every page with one placeholder and a settings file. By the end you'll have site-wide metadata, per-page head extras flowing through `settings.head` and front matter, and a proper canonical and robots story. Hreflang is covered in the [multilingual site tutorial](/docs/tutorial/multilingual-site/). The placeholder is the only piece of HTML you write yourself.
