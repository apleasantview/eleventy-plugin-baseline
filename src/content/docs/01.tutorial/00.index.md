...
The shortest path from an empty folder to a working Baseline site. One tutorial, read in order, ending with a site in two languages.

What it builds is the shape a fresh WordPress install hands you, and that is on purpose: you already know what that looks like, so no page here has to explain what the example site is meant to be.
