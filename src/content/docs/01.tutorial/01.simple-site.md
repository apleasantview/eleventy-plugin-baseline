...
Then the three helpers the scripts lean on: `rimraf` to delete `dist/`, `cross-env` to set environment variables, and `npm-run-all` to run one script after another. The first two work the same way on every operating system, which is the whole reason they are here rather than in the shell.
