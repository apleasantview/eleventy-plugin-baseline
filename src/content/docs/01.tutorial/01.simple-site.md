...
```json
{
	...
	"main": "index.js",
	"scripts": {
		"start": "rimraf dist/ && cross-env ELEVENTY_ENV=development BASELINE_URL=http://localhost:8080/ eleventy --serve",
		"build": "rimraf dist/ && cross-env ELEVENTY_ENV=production eleventy"
	},
	"keywords": [],
	"author": "",
	"license": "ISC",
...
Then the two helpers the scripts lean on, `rimraf` to delete `dist/` and `cross-env` to set environment variables, both of them the same way on every operating system. That last part is the whole reason they are here rather than in the shell.
...
Newer npm versions hold install scripts back until you approve them, so you may see a warning naming `sharp` and `esbuild`. Nothing in this tutorial needs either, so you can just ignore it or do `allow-scripts --all` if it bothers you.
