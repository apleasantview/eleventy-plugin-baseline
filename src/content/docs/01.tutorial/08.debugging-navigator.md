...
`inspectorDepth` is the default depth `_inspect` uses, set to 4. Per-call overrides take precedence: `{{ page | _inspect({ depth: 2 }) }}` uses depth 2 regardless of the global default.
...
`_snapshot` is a computed global with the shape `{ contentMap, pageContext }`. The content map is the per-build inventory of pages; `pageContext` is the per-page normalised view that the head and hreflang modules read from internally.
...
- Open `/debug-playground/` to see the filter and global output.
- (Optional) Open the navigator template at `/navigator-core.html` (the file keeps its historical `-core` filename; the URL follows). Expect verbose output.
