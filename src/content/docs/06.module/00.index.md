---
title: 'Modules'
slug: 'module'
description: "Baseline's five feature modules: assets, head, multilang, navigator, sitemap."
date: 2026-01-25
permalink: '/docs/module/'
---

Baseline ships as five feature modules. Each one wires a distinct concern into Eleventy and stays out of the way of the others. The pages below cover what each module does, when it activates, and the options it accepts.

If you are starting out, the [[concept-overview | concept overview]] introduces the three-layer model these modules sit on top of. The [[tutorial | tutorial]] walks through the common pairings.

## The five modules

{% stepsBlock compact %}

**[[assets]]**: wires JS (esbuild) and CSS (PostCSS) entry points into Eleventy, with inline filters for critical-path assets.

**[[head]]**: replaces a `<baseline-head>` placeholder with a sorted, deduped element list at transform-time.

**[[multilang]]**: language normalisation, per-page `locale`, translation map, and the i18n filters. Opt-in.

**[[navigator]]**: debug surface and public read surface. Globals, filters, a virtual page for inspecting render state, and the `_navigator` global (`{ graph, backlinks }`) for cross-page reads of the content graph.

**[[sitemap]]**: a single `/sitemap.xml`, or per-language sitemaps plus an index when multilang is active.

{% endstepsBlock %}

## Index

<ul>
{%- for item in collections.docs %}
{%- if item.data.topicSlug === topicSlug and item.url !== page.url %}
	<li><a href="{{ item.url }}">{{ item.data.title }}</a></li>
{%- endif %}
{%- endfor %}
</ul>

---

[[docs | Back to Docs]]
