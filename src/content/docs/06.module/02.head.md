...
The SEO payload is configured through `settings.seo` and per-page front matter, not through the head module directly. Baseline resolves it into a `seo` namespace and the driver emits the tags from there. See [[site-settings | Site settings]] for the keys.
