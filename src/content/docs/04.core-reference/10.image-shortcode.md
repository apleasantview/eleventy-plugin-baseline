...
Every key shown with its default, `outputDir` aside: that one is shown as the place the files end up, which is not always the place they are written. See [[image-shortcode#Caching | Caching]]. `src` and `alt` are required; the rest are optional.
...
Set them once at registration rather than on every call:

```js
eleventyConfig.addPlugin(
	baseline(settings, {
		media: { image: { widths: [640, 960, 1440], formats: ['avif', 'webp', 'jpeg'] } }
	})
);
```
