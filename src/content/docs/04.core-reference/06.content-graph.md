...
| Surface | Kind | Notes |
| ------- | ---- | ----- |
| `_node`, `_backlinks`, `_outgoing` | Cascade keys, per page | See [[globals \| Globals]] for shape. |
...
- [[seo-graph | SEO graph]] - the per-page SEO substrate; renders the graph's breadcrumbs as a `BreadcrumbList` in JSON-LD.
