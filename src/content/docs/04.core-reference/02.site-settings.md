...
- **languages** (`object | string[]`)
  - language map. Object form `{ en: {}, fr: {} }` or array shorthand `['en', 'fr']` (arrays are normalised to objects with empty entries; invalid entries are dropped, logged when `verbose: true`). Required for multilingual activation. Baseline reads three keys off an entry: `title`, `tagline`, and `locale` (the BCP 47 tag, e.g. `'en-US'`, that becomes `page.locale` for pages in that language):
  ...
  Anything else you put on an entry rides through untouched and is yours to read as `settings.languages[lang]`. Two of those are Baseline conventions: `contentDir`, the folder that language's content sits in, and `languageName`, the language's name in its own language. The plugin reads neither. They exist so a language switcher, or anything else that needs to find a language's content, has one place to ask.
