...
title: 'SEO graph'
slug: 'seo-graph'
description: 'A resolved per-page object built once during the cascade and read at transform-time: canonical URL, Open Graph and Twitter projections, and a JSON-LD @graph.'
date: 2026-06-07
seo:
...
The shape of the graph itself is not Baseline's invention. It follows the model of `seo-graph-core`, by Joost de Valk of Yoast.
...
- **In `_snapshot`**: every built graph lives under `_snapshot.seoGraph`, keyed by URL. No underscore, because it is the inspection snapshot surface. See [[globals | Globals]].
...
| `seo:`         | page front matter                | per-page _presentation_ overrides: `title`, `description`, `canonical`, `noindex`, the OG and Twitter fields. `seo.foo` wins over a bare `foo`. |
...
**Computed keys do not reach the graph.** Baseline builds the SEO graph from a global `eleventyComputed`, and Eleventy resolves global computed data before any page or template-level `eleventyComputed`. Your computed value is not there yet when the graph reads it.

This holds for every key in the table above, not just `schema`. A computed `ogImage`, `seo.description`, `type` or `topics` is silently absent: no error, no warning, and the tag either does not appear or quietly falls back to the site default.

Write them at the data layer instead: front matter, a directory data file, or `_data/`. Plain data resolves in time; computed does not.
...
```js
_seoGraph = {
	url, // resolved canonical URL; undefined when noindex, when settings.url is missing, or when there is no resolvable input
...
`_seoGraph` resolves to `null` on templates that opt out, on the same terms as page context: see [[page-context | Page context]]. Sitemaps, feeds, schema endpoints and the markdown-sibling renderer all take that route, because none of them needs a share card or a JSON-LD graph.
