...
| `head`   | Merged + deduplicated `link`, `script`, `style`, `meta` arrays from `settings.head` and page front matter. Dedupe keys: links on `rel` plus `href` (canonicals normalised first), scripts on `src`, styles on `href`, metas on whichever of `charset`, `name`, `property` or `http-equiv` identifies them. | `settings.head`, `data.head`                                                                            |
...
`page.translations` is grouped from the content graph, so a page only appears in a sibling's list if it is in the graph. A page carrying `_internal: true` or `baselineExcludeFromGraph: true` is not, and neither is a draft in a build that drops drafts. It is empty during the pre-pass, which is the build that produces the graph in the first place.

{% raw %}

```nunjucks
{% for variant in page.translations %}
	<a href="{{ variant.url }}" hreflang="{{ variant.lang }}">{{ variant.label }}</a>
{% endfor %}
```

{% endraw %}

`label` names the language ("Nederlands"), `title` and `description` name the sibling page, so a switcher can show either without a second lookup. The [[multilang | multilang module]] page has the fuller treatment, including the filters that predate this field.
