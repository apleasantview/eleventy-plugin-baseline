...
- **`_navigator.nodes`** is the per-page map from the content graph, keyed by URL. Each entry carries identity fields (`title`, `slug`, `description`, `lang`, `locale`, `date`, `url`, `breadcrumbs`) merged with extracted fields (`excerpt`, `headings`, `sections`, `images`). Identity comes from the page's `_pageContext`; the extracted fields come from the rendered HTML.
...
Pages can drop out of the graph two ways, and Eleventy's own collections opt-out is not one of them. The three flags and what each covers are on [[content-graph | Content graph]].
