...
Headings are slugified for reading rather than for filing, which matters once a heading names an API. Dots, slashes and underscores become word boundaries instead of disappearing, and symbols are dropped rather than spelled out:
...
| `Add a schema.js file` | `add-a-schema-js-file` |
| `The <head> placeholder` | `the-head-placeholder` |

{% endtableBlock %}

Accents are transliterated, so an anchor stays ASCII whatever language the page is in: `Le fossé pédagogique` becomes `le-fosse-pedagogique`. camelCase is left alone, so `inlineESbuild` stays `inlineesbuild` rather than being split.
...
Every part inherits the page's front matter, so every part carries the same description and slug. A wikilink to that slug lands on part one, and only part one joins a collection.

**The `<title>` and `og:title` are the exception.** From part two onward they lead with the part's label, so `Data files: A long guide` rather than the page title repeated. The label leads so that truncation eats the tail rather than the name of the part. Part one always keeps the page title as written, labelled or not.

Repeated descriptions across the parts are fine. Google does not ask paginated pages for distinct ones, and each part canonicalises to itself. There is currently no way to author a per-part description.
