...
- **translation-map store** - populated by the multilang module's collection; read by the wikilinks plugin while markdown renders, to resolve `[[slug:lang]]`.
- **translation-index store** - the same relationships grouped from the [[content-graph | content graph]]; read by the head module at transform-time to emit hreflang alternates. Two stores because the map exists during the pre-pass and the graph-built index does not.
