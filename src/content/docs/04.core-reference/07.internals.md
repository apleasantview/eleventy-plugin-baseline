...
- **translation-index store** - the same relationships grouped from the content graph; read by the head module at transform-time to emit hreflang alternates. Two stores because the map exists during the pre-pass and the graph-built index does not.
