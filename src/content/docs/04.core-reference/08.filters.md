...
Accepts the same options as the `assets.esbuild` config, but does not inherit it: what you pass is merged over the processor's defaults (`minify: true`, `target: 'es2020'`), whatever `assets.esbuild` says. Errors are non-fatal: a `<script>/* Error processing JS */</script>` placeholder is returned instead, and the build continues.
