...
Two arguments: site identity, then runtime behaviour. The `config` re-export at the bottom is the unusual part. Eleventy reads its directory configuration before any plugin runs, so Baseline ships that block as a separate export and asks you to forward it. Skip the re-export and the virtual directories Baseline registers (`assets`, `public`) won't be there, and a few modules will get quietly confused. The [[config-export | config-export reference]] has the full version.
...
| `navigator`           | dev only | Gates the debug virtual page. The `_navigator` read surface and debug filters/globals are always available.        |
