...
- **The page context.** A normalised per-page object built at cascade-time and cached for transform-time. Shape: `{ site, page, entry, query, meta, render, head }`. The user-visible surface is the `_pageContext` global; the [[page-context | page-context reference]] covers the full shape.
...
The full per-module reference lives in the [[module | modules]] chapter.
