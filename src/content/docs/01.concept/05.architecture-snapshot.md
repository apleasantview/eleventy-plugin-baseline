...
- **navigator**: debug tooling and the public read surface for plugin-produced cross-page data (`_navigator.graph`, `_navigator.backlinks`). The debug virtual page is on in development by default; the read surface is always available.
...
- **navigator** exposes the `_runtime` and `_ctx` globals, the inspector filters (`_inspect`, `_json`, `_keys`), the `_snapshot` debug surface, and the `_navigator` global (`{ graph, backlinks }`) for cross-page reads of the content graph.

  The snapshot reads from runtime stores so you can drop {% raw %}`{{ _snapshot.contentMap | _json }}`{% endraw %} into a template and see the build's current state. The `_navigator` global pairs with `data._graph` and `data._backlinks` (per-page records) for the same substrate at different scopes.
