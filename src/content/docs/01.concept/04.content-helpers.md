...
- `{% image %}` is always registered. `eleventy-img` is a peer dep that ships with Baseline.
...
Alt text matters. The shortcode warns when it is missing (use an empty string for decorative images). The [image-transform how-to](/docs/how-to/image-transform/) walks through the responsive pipeline.
