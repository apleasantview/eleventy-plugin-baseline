...
export default {
	...
	url: process.env.BASE_URL, // origin only, with protocol; the single source for every absolute URL
...
- `url` should be absolute, with protocol, and origin only. Everything absolute reads it: canonicals, sitemap entries, structured data, and the link rewriting `HtmlBasePlugin` does inside your pages. Subpath deployments belong in `pathPrefix`, which Eleventy layers on top; put the path in both and you get it twice.
- Baseline warns once at startup if `url` is missing, then omits canonicals rather than guessing an origin. The environment variable it comes from is yours to name, since Baseline only ever reads `settings.url`.
