...
The folder structure that keeps Baseline out of your way. If you've read [[what-is-baseline | what Baseline is]] and [[how-baseline-works | how Baseline works]], you already have the shape. This page is the practical companion: where files go, which environment variables matter, and the few seams Baseline expects to own.
...
- **Directory configuration.** Re-export `config`, or spread it. The four Eleventy keys (`input`, `output`, `includes`, `data`) are yours to change; Baseline reads back whatever Eleventy resolves. The two Baseline adds, `dir.assets` and `dir.public`, need to survive any override: drop `public` and the on-disk folder falls back from `static/` to `public/` with nothing to tell you. See [[config-export]] for the per-key detail.
