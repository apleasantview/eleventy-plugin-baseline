...
description: 'Every page has two layers: structure in a sibling *.11tydata.js, content metadata in front matter. A way of using the data cascade, not a Baseline rule.'
date: 2026-05-02
---
...
Extend `section` to `['blog', '2026']` and every page in the folder gets `/blog/2026/<slug>/` without each post having to spell out its own path. The array is the path.
...
The signal comes from two places already in the cascade. `data.lang` is set per language tree (typically in `en.11tydata.js`, `nl.11tydata.js`, and so on). `data.settings.defaultLanguage` lives in `_data/settings.js` and names the language that owns the root. What those two keys do beyond the permalink is the [[multilang | multilang module]]'s subject.
...
- **The data file isn't a page.** The `inputPath.includes('11tydata.js')` check returns `false` from the permalink. Baseline already adds `**/*.11tydata.js` to Eleventy's ignores, so the check rarely fires; it is cheap insurance that keeps the function correct on its own terms, and it travels if you lift it into a project without Baseline.
- **Paths use a leading slash.** Without it, the permalink resolves relative to the input subdirectory rather than the site root.
- **A permalink function cannot read an `eleventyComputed` value.** Computed data resolves after the cascade the permalink function runs in, so anything from `eleventyComputed` arrives as `undefined`. Build a URL from it and the function warns, returns `false`, and the template's own permalink wins instead, so the site looks right and only the console disagrees. The warning names the template rather than the data file that produced the value, which is what makes this one expensive to find. Read the raw front-matter and structural keys, as the function above does, and compute inside the permalink rather than before it.

---

## Opting out

A page can exist and render without showing up in indexes. Two keys cover the common cases, one Baseline and one Eleventy-native, and either works from front matter or the data file:
...
One is Baseline's, one is Eleventy's, and they are not the same scope: a page can sit outside every collection and still be in the graph. Which is which, and the third flag for synthetic templates, is on [[content-graph | Content graph]].

The organisational point is that both belong wherever the decision belongs. A single odd page declares it in front matter; a whole folder of them declares it once in the data file.
