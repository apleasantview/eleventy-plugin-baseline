...
Templates see one slice of state: the **`_baseline`** global, carrying `name` and `version` alongside `options`, `env`, `features` and `paths`:

- `env`: the package name and the resolved `ELEVENTY_ENV` mode.
- `options`: the resolved `state.options`, in the shape the plugin settled on.
- `features`: booleans only, one per module, plus a derived flag for whether `eleventyImageTransformPlugin` is loaded.
...
- **The translation map, and the translation index.** Which pages are translations of which, held twice over. The map is written by `multilang`'s collection and read by wikilinks while markdown renders; the index is grouped from the content graph and read by `head` for hreflang. Two of them because markdown renders during the pre-pass, where the graph does not exist yet.
...
The point of runtime is that **modules read through getters, never by direct event subscription**. A module asks "what are the translations right now?" and the runtime resolves it.
...
- The **registry** is the scope primitive (per-config cache, values map, listener dedup). It is the substrate the content-map store, the two translation stores, and the page-context registry sit on.
...
- **Stores** are the convention for runtime singletons attached to a registry scope (the content-map store, the two translation stores, and the slug index wikilinks resolves against).
...
- **navigator**: runtime-introspection tooling, and the read surface for the [[content-graph | content graph]]. The `phpinfo()`-style virtual page is on in development by default; the read surface is always available.
- **sitemap**: XML sitemap generation.

Each is registered through a small declarative registry inside the plugin entry point. `multilang` and `sitemap` carry an activation condition read from `state.options`; the other three always load, and where they have a switch (the navigator's virtual page) it gates one surface rather than the module.

They receive a uniform **module context** object: `{ env, state, runtime, directories, helpers, log, snapshots, resolvePageContext, resolveSeoGraph }`. The last two are handed over only to the modules that declared they consume them, which today means head alone; every other module gets `null` there.

That object is the module's only window onto the plugin; nothing else is in scope. Modules never share state directly.

If `head` needs the translation index, it reads it from `runtime`. If `sitemap` needs to know which languages exist, it reads `state.settings`.

The contract is small and uniform on purpose. The full per-module reference lives in the [[modules | modules]] chapter.
...
- **head** runs in two stages. At cascade-time, the page-context builder produces a normalised `head` seed object from `state.settings.head` and the page's own front matter.
  ...
  At transform-time, a PostHTML plugin reads those seeds, plus the translation index (when `multilang` is active) for hreflang alternates, and replaces `<baseline-head>` with a capo-sorted, deduped element list.

- **multilang** populates both translation stores and adds the `translationsMap` collection, the translation filters, and the computed `page.lang` / `page.locale` / `page.translationKey` / `page.isDefaultLang` / `page.translations` fields.

  Activation requires explicit opt-in: `options.multilingual: true`, plus `settings.defaultLanguage` or `settings.defaultLocale`, plus a non-empty `settings.languages` map.

- **navigator** exposes the debug globals and filters, the `_snapshot` surface, and the site-wide read surface for the content graph.

  The snapshot reads from runtime stores so you can drop {% raw %}`{{ _snapshot.contentMap | _json }}`{% endraw %} into a template and see the build's current state. The site-wide surface and the per-page cascade keys are the same substrate at different scopes; [[globals | Globals]] has both shapes.

- **sitemap** registers virtual templates that walk `collections.all` to generate the XML, and (when multilang is on with more than one language) produces per-language sitemaps plus an index.
...
- The [[modules | modules]] chapter for what each feature plugin does and the options it accepts.
