...
description: 'Integrate the official eleventy-base-blog starter with Baseline: what moves, what gets deleted, what comes back, and the failures a green build hides.'
date: 2026-08-25
seo:
...
What changes is who keeps the wiring working. The bundles, the head, the feed metadata and the sitemap template are yours to maintain today. Afterwards most of them arrive as a dependency you update, and the code that did the same job leaves the repository.
...
"Here is what Eleventy can do" is a coherent goal. It is a different job from "here is a blog you can start writing in", and most people clone it for the second. That is worth knowing before the config page, where a lot comes out at once. You are turning a showcase into an opinionated project, and the things you delete were exhibits.
...
This guide was written against `@apleasantview/eleventy-plugin-baseline@0.1.0-next.45` and `eleventy-base-blog` v9.0.0.

The reference implementation is the [`0.1.0-next.45` branch of eleventy-baseline-blog](https://github.com/apleasantview/eleventy-baseline-blog/tree/0.1.0-next.45), whose commits follow the steps below one for one.
...
{% detailsBlock "View `src/_data/settings.js`" %}
...
`author` is the only key here Baseline does not read itself. It stays because the feed and the templates use it.
...
{% detailsBlock "View the two directory data files" %}
...
Same shape, one different return, plus the two keys posts need.
...
This is the step the rest of the guide is arranged around. Patching the old config in place leaves you with registrations nobody has had to justify, and those are the ones that quietly stop being right when a module changes underneath them. Starting from nine lines makes the build ask you about each one.
...
2. **Delete the per-page bundle markup** in `base.njk` and `post.njk`, along with both `addBundle` calls. The assets module owns `dist/assets/`, and two things writing there is one too many. Rehome the per-page CSS, on the last page, picks up what the bundles were carrying.
...
{% detailsBlock "View the whole config at the end of this step" %}
...
Four registrations, in file order rather than the order the build named them.
...
{% detailsBlock "View the three registrations and their imports" %}
...
That is the integration. What is left is finding out whether it worked, and the exit code will not tell you.
...
It will pass. The most useful thing this integration taught is that Eleventy exits zero on a site that is quietly broken, so go and look at `dist/` rather than at the exit code:
