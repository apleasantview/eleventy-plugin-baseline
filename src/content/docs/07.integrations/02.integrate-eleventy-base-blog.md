...
Adding Baseline to `eleventy-base-blog` is mostly subtraction. The config goes from 168 lines to 84, sixteen registrations to nine, and `dependencies` ends up holding exactly one package. This guide walks the whole integration in the order it was actually done.
