...
This article is planned for revision, its content is stale.
...
- You have `eleventy-base-blog` cloned and running (`npm install`, `npm start` works).
- Eleventy 3.x; `package.json` has `start` and `build` scripts.
- `settings.url` will be set via env (`BASELINE_URL`) for production.
...
## Wire Baseline into `eleventy.config.js`
...
- Set `base` to `process.env.BASELINE_URL`.
...
## Add a local `.env` for development
...
In CI and production, set `BASELINE_URL` to your live origin (e.g. `https://www.example.com`); Baseline uses it for canonical URLs and the sitemap.

---

## Move Eleventy Base Blog folders under `src/`
...
##Migrate the starter's head children
...
**Per-page bundled CSS → `getBundle("css")` style block.** This is the non-trivial migration. The starter collects per-page CSS through `eleventy-plugin-bundle` and inlines the result. Inline `<style>` does have a head-extras shape: an entry in `head.style[]` with a `content` key renders as a real `<style>` element in the head, front matter included, so per-page CSS can go there.

```yaml
head:
  style:
    - content: '.callout { border-left: 3px solid; }'
```

If the bundle is what you want to keep, two approaches work depending on what it is for:
...
## Organise content and set permalinks
...
// blog.11tydata.js
export default {
	// the starter's existing tags and layout keys stay
...
## Use Baseline's inline-asset filters where it makes sense
...
## Optional: navigator and debug filters

    - Leave `navigator` alone if you want the navigator page in development only, which is the default. Setting `navigator: true` turns it on everywhere, builds included.
