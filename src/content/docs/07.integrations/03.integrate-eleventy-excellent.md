...
description: 'Integrate the eleventy-excellent starter to Baseline plugin.'
date: 2026-09-04
seo:
...
Neither project took that from the other. They are answers to the same question, which is what a site needs settled before you can start writing, and there are only so many good answers.

## Before you start

The overlap is why this guide is shaped the way it is. Most of the conversion is renaming, and Baseline does not run at all until over half the steps are done. You are not replacing the starter's thinking so much as moving it onto conventions that already match, then handing over the parts a plugin can keep up to date for you.

The longest chapters here are the ones about conventions rather than the ones about the plugin, and that is the shared ground showing. There is more to say about where files sit and what they are called than about the thing that reads them.

Where the two disagree the guide says so. One bundle per page becomes one per directory, inlined CSS becomes linked, and seven hand-written head partials become a placeholder. The first two are arguable, and the numbers are here so you can argue with them.

## Prerequisites

The starter cloned and running, Node 22 or newer, and somewhere to set `BASELINE_URL` on your host. Written against `@apleasantview/eleventy-plugin-baseline@0.1.0-next.46` and eleventy-excellent 4.7.1.

## The shape of it

The first nine steps do not use Baseline. They are structure, naming and config shape, and the site builds identically to upstream all the way through them. The plugin is installed but never runs, so you can do all of it before deciding whether you want it, and stop at the seam with a tidier project and nothing else changed.

The eight steps after the seam each hand a surface over: the sitemap, the identity graph, the share cards, the head, the assets.

By the end, the dependency count is down from 45 to 42, `src/_includes/head/` is gone with its seven partials, the two schema templates are replaced by one data file, and the CSS and JS build events are replaced by entry points. The conversion removes more than it adds.
...
## Helper packages and build scripts

One dependency, and the scripts rewritten to Baseline's shape.
...
Get that filename exactly right. Eleventy reads `<dirname>.11tydata.js` and nothing else, so `common.11ty.data.js`, one dot out, is an ordinary module no build ever loads. Nothing warns, and all three keys quietly do not apply.
...
### The sitemap goes in this same commit
...
The third line is the one to read. Baseline registers its own `image` shortcode only where nothing has claimed the name, so yours keeps it. That is the general rule from here on: Baseline takes surfaces you have not already spoken for, and says so when it does not.
...
Both are the same shape: correct-looking code that works because nothing is reading its output yet.
...
Baseline composes the head from data, which is what the partials were doing by hand. Three of the starter's own tags are wrong, and none of the three has anything to do with Baseline: the canonical and `og:url` carry a double slash from concatenating `meta.url` with `page.url`, `og:locale` is `en_EN`, which is not a locale, and `article:author` appears on all 34 pages, including ones whose `og:type` is `website`.
...
Baseline also emits five things the starter never did: `charset`, `viewport`, `<title>`, `twitter:card` and the JSON-LD graph.
...
This is a workaround rather than a pattern. If the ordering is fixed upstream, the four tags come out.
...
Baseline compiles one bundle per directory, from the `index` file it finds there. `global/index.css` is the starter's existing entry renamed and keeps its layer order and its globs untouched, `local/index.css` is new and replaces the eleven per-page bundles, and the JS entry is two lines because most of this starter's JavaScript stays page-scoped.
...
The starter declares its assets where they are used, with a `{% raw %}{% css %}{% endraw %}` or `{% raw %}{% js %}{% endraw %}` block wrapping an include. Baseline's model is one bundle per directory instead, so those blocks do not all become the same thing. The CSS ones are deleted, the JS ones are translated, and two things move to the config.

## The eleven CSS blocks are deleted outright

Every one of these looks the same, and every one goes:
...
Delete the block, not the stylesheet. The file it included is one of the ten now listed in `local/index.css`, so the CSS still ships, it just ships on every page instead of the pages that use it. That is the per-page scoping you are giving up, and it is the one decision here you might reasonably refuse.
...
**`@import-glob` only fires in the entry file.** Do not put a thin entry point above a stylesheet that uses it. Globs one level down are never processed, silently drop everything they matched, and survive into the output as dead text. `global/index.css` globs three directories safely, because it is the entry itself. Had it been reached through a wrapper, those three lines would have shipped to the browser as text and taken every composition, block and utility with them. `local/index.css` lists its ten imports by hand for that reason: it could glob today, and it would break quietly the first time anything sat above it.

{% enddetailsBlock %}

## The five JS blocks become `inlineESbuild` calls

The JavaScript does not lose its scoping the way the CSS does. These blocks are genuinely page-scoped and stay that way. Swap the block for the filter, in place:
...
## Two more things move, and one gets added

The config loses the pre-build events and the CSS bundle, since the plugin does both now:
...
And the custom elements need a passthrough entry, because they are referenced by `<script src>` rather than bundled, so nothing compiles them:

```js
// eleventy.config.js, added to the existing passthrough map
'src/assets/js/components/': `assets/js/components/`
```

Their `<script src>` paths need updating too, in `custom-masonry.webc` and `base.njk`, since `src/assets/scripts/` is now `src/assets/js/`.

## Template syntax stops being substituted
...
## The dependencies that lost their importer

Four packages come out. The count goes from 45 to 42 rather than 41, because `npm-run-all` went in at the third step.

```bash
npm uninstall esbuild postcss-cli fast-glob sanitize-html
```

The first three have one importer each, all of them in code the assets step deleted. `sanitize-html` is a different case: nothing in the starter imports it, in any version. Justifying every remaining dependency once is how you find that kind of thing.

## What a green build will not tell you

Eleventy exits zero on a site that is quietly broken, and most of what goes wrong in this conversion is silent rather than fatal.
...
Every one of those is a count rather than a look, which is the shape of the whole list: a presence check passes on a page that is missing exactly the thing you changed.
...
**The starter's own prose.** `docs/` still describes mechanisms this conversion removes. That is the starter author's copy rather than the integrator's.
...
- [[head | Head module]]
- [[seo-graph | SEO graph]]
- [[integrate-eleventy-base-blog | Converting eleventy-base-blog]]
