...
Because it shares the same markdown-it instance as Baseline's amendments, inline-level extensions apply here too: `[[slug]]` wikilinks resolve, and `{#id .class}` attribute syntax works on inline spans. Block-level features (auto heading IDs) do not fire because `renderInline` skips the block parser.
