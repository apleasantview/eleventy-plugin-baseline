...
- **Reserved data keys** - nine reserved keys initialised to `{}` so module-namespaced data can merge cleanly. See [[globals | Globals]].
...
- **Wikilinks** - `[[slug]]` syntax in Markdown body content, resolved through a markdown-it plugin. Forward links only; backlinks are not generated.
