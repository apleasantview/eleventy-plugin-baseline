...
- **Markdown substrate** - three plugins applied through `amendLibrary('md', ...)`, each guarded by a `safeUse` helper that skips re-registering plugins a user already loaded: `markdown-it-attrs` (for `{#id .class}` syntax on any element), an auto-heading-ids plugin (assigns stable `id` attrs with WordPress-style dedup; manual ids win), and the `wikilinks` plugin (`[[slug]]` / `[[slug:lang]]` / `[[slug|alias]]` / `[[slug#anchor]]` resolution). See [[content-helpers | Content helpers]] for the author-facing syntax.
