...
- **Reserved data keys** - ten reserved keys initialised to `{}` so module-namespaced data can merge cleanly. See [[globals | Globals]].
...
- **Content-graph pre-pass** - sentinel-guarded inner build that walks every rendered page, writes `.cache/_baseline/content-graph.json`, and hooks the result into the cascade as `eleventyComputed._graph` (per-page record) and `eleventyComputed._backlinks` (per-page enriched backlinks list). The site-wide view is on the navigator's `_navigator` global (`{ graph, backlinks }`).
