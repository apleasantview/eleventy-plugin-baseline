...
- **`_navigator.nodes`** is the per-page map from the content graph, keyed by URL. Each entry carries identity fields (`title`, `slug`, `description`, `lang`, `locale`, `date`, `url`) merged with extracted fields (`excerpt`, `headings`, `sections`, `images`). Identity comes from the page's `_pageContext`; the extracted fields come from the rendered HTML.
