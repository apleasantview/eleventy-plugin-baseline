...
Pages flagged `eleventyExcludeFromCollections: true` (404, drafts, internal templates) drop out of the graph cleanly, as do pages flagged `baselineExcludeFromGraph: true` (a graph-specific opt-out used by showcase index pages that should render but stay outside the graph).
