...
They exist so module-namespaced data can merge cleanly without colliding with same-named filters. No module currently writes into them; if you see one of these in a `_data` dump, it's `{}` by design. `eleventyComputed._contentGraph` is held against a parked content-graph cascade hookup; the substrate runs but nothing is bound to the cascade yet.

Together with `_baseline`, `_pageContext`, and `_snapshot`, that is nine reserved keys. The full list lives in `INTERNAL_KEYS` in the entry point. See [[internals | Internals]].
