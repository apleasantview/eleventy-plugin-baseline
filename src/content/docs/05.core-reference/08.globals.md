...
- **`_navigator.graph`** is the full pages map from the content graph, keyed by URL. Each entry has `{ text, excerpt, headings, links, images }`.
...
- **`_graph`** is the current page's own record from the content graph: `{ text, excerpt, headings, links, images }`. Read it for the page's headings list, outgoing links, or first-paragraph excerpt.
- **`_backlinks`** is the page's enriched backlinks list: `Array<{ url, title?, excerpt? }>`. Each entry names a page that links to the current one. `title` is sourced from the linking page's front matter (when present); `excerpt` is the linking page's own first-paragraph excerpt. Both are optional. Empty array if nothing links here.
