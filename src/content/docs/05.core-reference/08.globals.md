...
- **`_backlinks`** is the array of edges where `to === page.url`: bare edge records, shape `{ internal, from, to, text, rel }`. Each entry names a page that links to the current one and the text of the link. Empty array if nothing links here.
