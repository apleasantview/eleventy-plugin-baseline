...
Baseline pre-registers nine reserved keys as empty objects so module-namespaced data merges cleanly and does not collide with same-named filters. The full list (`INTERNAL_KEYS`):

`_baseline`, `_assets`, `_head`, `_multilang`, `_navigator`, `_sitemap`, `_snapshot`, `eleventyComputed._pageContext`, `eleventyComputed._contentGraph`.

Of those, three are actually populated today (`_baseline`, `_pageContext`, `_snapshot`); the rest are reserved-but-empty. `eleventyComputed._contentGraph` is held against a parked content-graph cascade hookup; nothing is bound to it yet. See [[globals | Globals]] for what's populated and where.
