...
Each part is its own page with its own `<head>` and canonical, and each part canonicalises to itself. From part two onward the `<title>` leads with the part's label, so it reads `Data files: A long guide` rather than repeating the page title. Part one always keeps the page title as written.

Descriptions are still shared across the parts, which is fine. Google does not ask paginated pages for distinct ones.

Every part is in the sitemap, inheriting the page's `lastmod`, `changefreq` and `priority`. Parts deliberately carry no hreflang, since an alternate has to name the equivalent page and part one is not it.
