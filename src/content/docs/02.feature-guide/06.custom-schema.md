...
Author pieces at the data layer: front matter, a directory data file, or `_data/schema.js`. Schema placed under `eleventyComputed` is silently ignored. Baseline assembles the graph before page-level computed data resolves, so a computed `schema.pieces` never reaches it. Plain data resolves in time; computed does not. This is also why projecting a collection into pieces needs more than a computed key, and why that how-to is still pending.
...
The one rule that matters is **quote your keys and your string values**. It dodges the `@`-key trap, since `@` is a reserved indicator in YAML and bare `@type: Thing` errors while `"@type"` or `'@type'` is fine, which is why pasted JSON, already quoted, just works. It also avoids YAML's scalar coercion, where unquoted `name: yes` becomes a boolean and `name: on` likewise. Quoted values are immune to both.

Strip the quotes and you re-enter YAML's quirks. Baseline doesn't guard against that (see [On validation](#on-validation) below); the convention does the work a guard would.
...
## Arriving with a graph you already built

If your site already emits its own JSON-LD, whether hand-rolled or from another plugin, delete it when you adopt Baseline. Two `ld+json` blocks in one head is the most common way this goes wrong, and it goes wrong quietly: the build passes, both graphs are individually valid, and the page ships two sets of nodes with conflicting `@id`s for the same things.

Nothing warns you, because nothing can tell the difference between your second graph and a deliberate one. The tell is in the built output rather than the console, so check a page in `dist/` before and after:

```bash
grep -c 'application/ld+json' dist/index.html
```

If your old layer also wrote `og:` or `twitter:` meta tags, those go too, for the same reason.
