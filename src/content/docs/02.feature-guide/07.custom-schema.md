...
- It dodges the `@`-key trap. `@` is a reserved indicator in YAML, so bare `@type: Thing` errors while `"@type"` or `'@type'` is fine. Quote the key and the trap never appears, which is why pasted JSON, already quoted, just works.
- It avoids YAML's scalar coercion. Unquoted, `name: yes` becomes a boolean and `name: on` likewise; quoted values are immune.
