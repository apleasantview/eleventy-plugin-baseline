...
The site from the [[simple-baseline-site | simple site tutorial]], with Baseline installed, `eleventy.config.js` written, and the `start` and `build` scripts in `package.json`. Nothing here needs new options.

---

## 1) Add a page that links assets
...
## 5) Production build and inspect output

```bash
npm run build
```

Check `dist/assets/css/index.css` for the PostCSS output, and `dist/assets/js/index.js` for the bundled and minified JS. The difference from `npm start` is `ELEVENTY_ENV=production`, which is what turns cssnano and esbuild's minifier on.
