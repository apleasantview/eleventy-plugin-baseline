...
`settings.url` set. Canonical URLs are the one thing on this page that needs a real origin.

---

## Set the head options

Two options shape what the head module emits. Add them to the plugin call in `eleventy.config.js`:
...
The canonical is written for you, from the page's own URL and `settings.url`. The [[head | head module]] has the exact rules, including what the query string does and what happens when `settings.url` is missing.
...
## Noindex (site-wide and per-page)

Three knobs, in widening order of scope:

{% stepsBlock "compact" %}

Site-wide: set `noindex: true` in `_data/settings.js`. Every page gets a `robots` noindex tag, canonical links are dropped, and the sitemap still renders but lists nothing.

Per-page: set `noindex: true` in front matter, or `noindex: true` under `seo:` if you keep the presentation keys together. The page renders a `robots` noindex tag and drops out of the sitemap. The `seo:` spelling wins where both are set, which is how a page opts back in against a `noindex: true` it inherited from a directory data file.

Sitemap-only: set `sitemap: { ignore: true }` in front matter. The page renders normally; only the sitemap entry is removed.

{% endstepsBlock %}

The relationship is deliberate: `noindex` is the broader signal (don't index, don't list), `sitemap.ignore` is the narrow one (just don't list). The [[sitemap | sitemap module]] lists the same knobs from the sitemap's side, plus the fourth one that only affects collections.
...
The canonical points at whatever origin `settings.url` resolved to for this build. Getting that right per environment is its own subject: see [[deployment-checks | deployment checks]].
...
- [[head | Head module]] - every option and everything the module emits.
- [[site-settings | Site settings]] - the shape of `settings.head`.
- [[multilingual-baseline-site | Build a multilingual site]] - hreflang, and where the canonical story goes next.
- [[deployment-checks | Deployment checks]] - getting `settings.url` right per environment.
