...
The canonical points at whatever origin `settings.url` resolved to for this build. Getting that right per environment is its own subject: see [[deployment-url-checks | deployment URL checks]].
...
- [[deployment-url-checks | Deployment URL checks]] - getting `settings.url` right per environment.
