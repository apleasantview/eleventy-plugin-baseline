...
```js
url: process.env.URL; // one value, supplied per environment
```
...
- Hosted previews (optional, e.g. Netlify): `URL` points at production while `DEPLOY_PRIME_URL` points at the preview. Baseline reads neither. Pick whichever one you want the build to claim and assign it to `settings.url` yourself.

Baseline loads `dotenv` for you, so `process.env.URL` works in both `_data/settings.js` and `eleventy.config.js`. The variable name is yours: Baseline only reads `settings.url`, so whatever your host calls it, put it there.
