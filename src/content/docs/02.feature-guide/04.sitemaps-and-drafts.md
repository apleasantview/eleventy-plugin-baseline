...
- In any build: drafts are skipped. The switch is Eleventy's own `ELEVENTY_RUN_MODE`, which is `build` for anything that is not serving or watching, so a bare `npx @11ty/eleventy` drops them too. This is not the `ELEVENTY_ENV` your `build` script sets; that one governs asset minification and the navigator.
