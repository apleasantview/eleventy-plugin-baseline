...
- In dev (when `ELEVENTY_RUN_MODE=serve`), the shortcode uses Eleventy Image's `transformOnRequest` mode, which defers image processing to the first browser request and keeps server startup snappy. It does the same during the content-graph pre-pass, which renders every page but writes nothing, so your images are generated once per build rather than twice. Builds pre-generate everything as usual. If `transformOnRequest` fails, the shortcode falls back without it.
