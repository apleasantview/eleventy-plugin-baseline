...
- A `.env` for environment-specific values:
  ...
  ```
  Baseline reads `ELEVENTY_ENV` itself, to decide what to enable in development versus production. `BASE_URL` it never sees: the config below hands it to Baseline as `settings.url`, which is the one place everything absolute is anchored. The name is yours to pick. The other entries you might add are project-specific.
...
- Production build:  
  `BASE_URL` is the address the build claims as its own, so point it at the real one before shipping. For this exercise any absolute URL does the job, `https://www.example.com/` included. Then run:
  ```bash
  npm run build
  ```
  Inspect `dist/` for the final files, and check that the canonical link and `dist/sitemap.xml` both carry the URL you set. Set `BASE_URL` back to `http://localhost:8080/` before returning to `npm start`.
