...
- `baseline()` takes two arguments. `settings` is the site identity (title, url, languages, head extras), written once in `src/_data/settings.js` and imported here; `options` is runtime behaviour (which modules are on, log verbosity, per-module options).
