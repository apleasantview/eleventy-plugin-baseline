...
This is the deeper read. If you just want to ship, the [quickstart](/docs/introduction/quickstart/) is enough; this page is for when you want to understand what is going on under it.
...
The [globals reference](/docs/reference/globals/) covers the exact shape; the [plugin-entrypoint reference](/docs/reference/plugin-entrypoint/) covers the option list that feeds into it.
...
- **The page context.** A normalised per-page object built at cascade-time and cached for transform-time. Shape: `{ site, page, entry, query, meta, render, head }`. The user-visible surface is the `_pageContext` global; the [page-context reference](/docs/reference/page-context/) covers the full shape.
...
These are internals you can mostly ignore. The [internals reference](/docs/reference/internals/) covers them in case you hit one through the navigator.
...
The full per-module reference lives in the [modules](/docs/modules/) chapter.
...
- **navigator** exposes the `_runtime` and `_ctx` globals, the inspector filters (`_inspect`, `_json`, `_keys`), and the `_snapshot` debug surface. The snapshot reads from runtime stores so you can drop {% raw %}`{{ _snapshot.contentMap | _json }}`{% endraw %} into a template and see the build's current state.
...
- The [reference index](/docs/reference/) for the lookup view of options, globals, filters, and the page context.
- The [page-context reference](/docs/reference/page-context/) for the exact shape of the per-page object.
- The [internals reference](/docs/reference/internals/) for the registry, stores, and virtual directories.
- The [modules](/docs/modules/) chapter for what each feature plugin does and the options it accepts.
