...
- **Runtime** is state in motion, plus a per-page context and a registry that lets the modules read what they need without reaching across to each other.
...
- **[[multilang]]**: directory-based multilingual support. Per-language collections, hreflang, a translation map, and translation filters (it wraps Eleventy's `I18nPlugin`). Opt-in.
...
- A **drafts preprocessor**: any template with `draft: true` in front matter renders while the dev server is running and is dropped from a build.
