...
- **[[navigator]]**: debug surface and public read surface. Globals and filters for inspecting template data, an optional virtual page (on in development by default), and the `_navigator` global (`{ graph, backlinks }`) for cross-page reads of the content graph.
