...
title: 'Filters'
slug: 'filters'
permalink: '/docs/reference/filters/'
...
Registered by the [assets module](/docs/modules/assets/), which is always active.
...
Registered by the [multilang module](/docs/modules/multilang/), available only when multilingual mode is active. All three operate on `collections.translations` and a page's `locale.translationKey`.
...
Registered by the [navigator module](/docs/modules/navigator/) when active (default in development). All three are debugging helpers; treat them as dev-only.
