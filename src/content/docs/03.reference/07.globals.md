...
title: 'Globals'
slug: 'globals'
permalink: '/docs/reference/globals/'
...
## `date` (Nunjucks global)

A small date helper exposed as a Nunjucks global. One method today:

{% raw %}

```nunjucks
{{ date.toUTCISO(page.date) }}

```

{% endraw %}

Returns the value as a UTC ISO string with milliseconds suppressed, or `''` if the input is missing or unparseable. Backed by Luxon. Source: [`_baseline/core/global-functions/date.js`](_baseline/core/global-functions/date.js).
