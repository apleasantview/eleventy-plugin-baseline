...
title: 'Site settings'
slug: 'site-settings'
permalink: '/docs/reference/site-settings/'
...
Settings flow through `state.settings` and surface in templates as `_pageContext.site` (and as `settings` in the data cascade when you keep them in [`_data/settings.js`](/docs/introduction/quickstart/), which is the convention used throughout the docs).
...
- **head** (`object`)
  ...
  Pages can add their own entries through the same shape in front matter; Baseline merges and deduplicates. See the [head module](/docs/modules/head/) for the merge rules.
...
See the [multilang module](/docs/modules/multilang/) for what activation enables (per-language collections, hreflang, i18n filters).
