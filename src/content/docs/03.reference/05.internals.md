...
title: 'Internals'
slug: 'internals'
permalink: '/docs/reference/internals/'
...
The asymmetry between the virtual `public` key and the on-disk `static/` folder is documented in [Config export](/docs/reference/config-export/).

## Driver seam

A **driver** is a swappable rendering strategy inside a module. Today the head module uses one driver: `posthtml-head-elements`, which composes the head with PostHTML and the `posthtml-head-elements` plugin. The driver is selected by import (`import { renderHead } from './drivers/posthtml-head-elements.js'`), not by user option.

The seam exists as forward-looking architecture: a future driver (direct DOM, a different head-elements engine, Cheerio) can be slotted in without changing how cascade-time data flows into the renderer. There is nothing to configure today; the seam is named so it has somewhere to live when a second driver shows up.

Source: [`_baseline/modules/head/drivers/`](_baseline/modules/head/drivers/).
...
Of those, three are actually populated today (`_baseline`, `_pageContext`, `_snapshot`); the other five are reserved-but-empty. See [Globals](/docs/reference/globals/) for what's populated and where.

The `_internal: true` opt-out is documented on [Page context](/docs/reference/page-context/); it skips a template from the page-context registry.
