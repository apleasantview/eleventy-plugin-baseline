...
2. Keep `settings.url` clean (origin only). The [Deployment URL Checks tutorial](/docs/tutorial/deployment-url-checks/) covers the env shape; canonicals and sitemap entries combine `settings.url` with `pathPrefix` automatically.
