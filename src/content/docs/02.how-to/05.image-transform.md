...
- When using the Image Transform plugin alongside Baseline's `image` shortcode, the shortcode adds `eleventy:ignore` to its output automatically so the transform won't rewrite it. No extra configuration needed.
