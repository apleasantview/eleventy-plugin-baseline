...
If you don't already have a project to integrate into, the [Simple Baseline Site](/docs/tutorial/simple-baseline-site/) tutorial is the lighter starting point.

{% alertBlock "info" %}

A completed integration is [available on GitHub](https://github.com/apleasantview/eleventy-baseline-blog).
...
2. Wire Baseline into `eleventy.config.js`.
   ...
   - Keep the `posts` collection clean of `11tydata.js` after Baseline is wired:
     ```js
...
4. Move Eleventy Base Blog folders under `src/`.
   ...
   The last move has an asymmetry worth knowing upfront. Baseline's directory contract sets `public: 'static'`, so the virtual key on `eleventyConfig.directories` is `public` but the folder on disk stays `static/`. The [config-export reference](/docs/reference/config-export/) covers the same split.
...
- Add recommended build scripts and helper packages: [Build Scripts and Cleanup](/docs/how-to/build-scripts-and-cleanup/).
