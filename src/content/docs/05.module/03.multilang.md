...
- **Build-time.** Adds `I18nPlugin`, registers the translation filters, registers the computed per-page language fields, normalises the language config through [`normalizeLanguageMap`](https://github.com/apleasantview/eleventy-plugin-baseline/blob/main/_baseline/core/utils/normalize-language-map.js).
...
1. **Normalise the language config.** [`normalizeLanguageMap`](https://github.com/apleasantview/eleventy-plugin-baseline/blob/main/_baseline/core/utils/normalize-language-map.js) accepts an object map or an array of strings, lowercases the keys, drops invalid entries (logged when `verbose: true`), and returns the normalised object.
...
| `settings.defaultLanguage` | `string`             | Short default-language code (`'en'`), an alias for the above. Either key serves every consumer: the sitemap module resolves the default through the same resolver this module uses, and the breadcrumb builder reads the computed `page.isDefaultLang` rather than the setting. All of them assume the convention that the default language sits at unprefixed URLs and the others under `/<lang>/`. Writing those permalinks is still yours. |
| `settings.languages`       | `object \| string[]` | Map of language codes to per-language overrides, or a flat array of codes. Arrays are normalised to objects with empty entries. Six keys are read off an entry: `title`, `tagline`, `description`, `locale`, `languageName` and `homeLabel`. |

{% endtableBlock %}

An entry can be empty, and `{ en: {}, sv: {} }` is a valid configuration that activates the module. It is rarely the one you want: with nothing on the entry, pages in that language fall back to the site-level `title`, `tagline` and `description`, the language switcher labels itself with the bare code (`sv` rather than `Svenska`), the first breadcrumb reads `Home` in every language, and `page.locale` falls back to `defaultLocale`. Nothing warns, because nothing is wrong. See [[site-settings#Keys | Site settings]] for what each key does and what it falls back to.
