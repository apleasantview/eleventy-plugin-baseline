...
title: 'navigator'
slug: 'navigator'
description: 'Two hats: a phpinfo()-style page for current build state, and the _navigator read surface for cross-page data (nodes, edges, backlinks).'
date: 2026-04-15
---
...
As the **public read surface**, it registers the `_navigator` Nunjucks global with shape `{ nodes, edges, backlinks }`: the site-wide view of the [[content-graph | content graph]]. The per-page view of the same substrate is on `data._node`, `data._backlinks`, and `data._outgoing`. Both are documented in [[globals | Globals]].
...
- **Cascade-time.** `_snapshot` resolves on every page; its `contentMap`, `pageContext` and `seoGraph` fields are read from the runtime stores and registries.
...
3. **Register `_navigator`.** Added as a global thunk reading `{ nodes, edges, backlinks }` from `runtime.contentGraph`. All three fall back to an empty object when the graph is unavailable, so templates can read it without guarding. Note that this includes `edges`, which is an array once the graph exists: check the length of a `for` rather than the array itself if you run during the pre-pass.
...
- **`_navigator`** returns `{ nodes, edges, backlinks }`, the cross-page read surface for plugin-produced data. Use it to paginate over backlinks for "what links here" subpages, or to walk `nodes` and `edges` for related-page lists and link audits. The field-by-field shape is on [[globals | Globals]].
...
Every page receives a computed `_snapshot` with three fields, read from Baseline's runtime stores:

- **`contentMap`**: the inputPath ↔ url lookup Baseline builds during the cascade. The page-context builder reads it when resolving a canonical from an input path.
...
- **`seoGraph`**: the seo-graph registry's snapshot, keyed by URL. The resolved canonical, OG/Twitter projections and JSON-LD the head emits. See [[seo-graph | SEO graph]].
