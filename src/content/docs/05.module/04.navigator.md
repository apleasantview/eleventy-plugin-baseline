...
3. **Register `_navigator`.** Added as a global thunk reading `{ nodes, edges, backlinks }` from `runtime.contentGraph`. All three fall back to an empty value of their own type when the graph is unavailable, `{}` for `nodes` and `backlinks` and `[]` for `edges`, so templates can read any of them without guarding. During the pre-pass that is what you get, since the pre-pass is the build that produces the graph.
