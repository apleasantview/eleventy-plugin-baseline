...
title: 'head'
slug: 'head'
description: "Drop a <baseline-head> placeholder in your layout and get the full head: title, description, canonical, hreflang, Open Graph, Twitter, and JSON-LD."
date: 2026-04-15
seo:
...
During the data cascade, it builds seeds: the per-page values the renderer needs (title, description, robots, canonical, head extras, locale). At transform-time, after Eleventy has rendered HTML, the PostHTML driver reads those seeds, builds hreflang alternates from the translation index when relevant, dedupes, sorts the elements by capo.js weights, and swaps the placeholder for the final `<head>`.
...
- **Transform-time.** The PostHTML driver looks up the seeds for the current page, builds hreflang alternates from the translation-index store, emits the element list, dedupes meta and link entries, capo-sorts, and replaces `<baseline-head>` with a real `<head>`.
...
The renderer is PostHTML, which Baseline reaches through `htmlTransformer.addPosthtmlPlugin`, so it is Eleventy's rather than a dependency of ours. Baseline owns the seed-building, the dedupe, the sort and the placeholder replacement.

---

## The placeholder

`<baseline-head>` is not a tag that goes inside the head. It _is_ the head. Place it where the `<head>` element would go, as a sibling of `<body>`, and the driver replaces the whole element with a real `<head>` containing the composed list:
...
Do not wrap it in a `<head>` of your own. Baseline emits the opening and closing tags along with everything that belongs between them.

If the placeholder is missing, the driver has nothing to match and the page ships without a `<head>`. Silently: no error, no warning, no partial output. The warning you may be thinking of is a different case, a page with no seeds, covered under How it works below.

---

## Emitting a tag yourself

Two positions, and they do opposite things.

**Inside the placeholder, content is discarded.** The driver replaces the whole element with a fresh `<head>`, so anything you nest in there never reaches the output. No warning: the tag is gone and the page is otherwise fine.

**Below the placeholder, as a sibling, the tag arrives untouched.** This is the escape hatch:
...
It works because of the HTML parser rather than anything Baseline does. In the "after head" insertion mode, a `base`, `link`, `meta`, `script`, `style` or `title` element found between the head and the body is folded back into the head. So the tag lands where you wanted it, having gone nowhere near Baseline's composition.

Reach for it when `settings.head` cannot express what you need. The worked case is the one above: two `theme-color` metas that differ only by `media`. Baseline's meta dedupe keys on `name`, so through `settings.head` one of the two would silently win. As siblings they never meet the dedupe at all.

The trade is that these tags skip everything: no dedupe, no capo sorting, no absolute-URL rewriting. That is the point of the seam, and it is also the reason to use `settings.head` for anything that does not need it.
...
3. **Emit.** Standard meta first (charset, viewport, title, description, robots, canonical, optional generator). Then user extras from `seeds.head`. Then hreflang alternates from the translation index. Then the SEO payload (Open Graph, Twitter, JSON-LD), read from the resolved `seo` namespace through its own handle rather than from `seeds.head`.
4. **Dedupe.** Meta and link entries are deduplicated; later entries (page-level) win over earlier ones (site-level). A meta tag's `media` attribute is part of its identity, so a light and a dark `theme-color` both survive.
...
- **Robots.** Always emitted. An indexable page gets `<meta name="robots" content="index, follow, max-snippet:-1, max-image-preview:large, max-video-preview:-1">`; a page carrying `noindex` (its own, its `seo.noindex`, or `settings.noindex`) gets `<meta name="robots" content="noindex, nofollow">` instead.
...
An entry with nothing to key on is dropped rather than emitted: a `<meta>` carrying none of `charset`, `name`, `property` or `http-equiv`, or a `<link>` missing either `rel` or `href`. If a tag you added never appears, that is the first thing to check.

---

## Hreflang

When the multilang module is active and the page has a `translationKey`, the driver builds an `<link rel="alternate" hreflang="...">` for every translation listed in the translation index. Nothing to configure on the head side. If the page has no `translationKey`, no alternates are emitted.

---

## Tips

- Keep `<baseline-head>` where the `<head>` belongs, as a sibling of `<body>`. The driver replaces the placeholder wherever it sits, so a misplaced one produces a `<head>` in the wrong part of the document.
...
- [[multilang | multilang module]] for the translation relationships that power hreflang.
