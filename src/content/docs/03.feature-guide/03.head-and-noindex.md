...
Baseline computes the canonical URL from `page.url`, `settings.url`, and `pathPrefix`. If `settings.url` is missing, the canonical falls back to a relative URL.

To override per page, add a `head.link[rel=canonical]` entry in front matter; it will dedupe over the computed one. There is no top-level `canonical` field on front matter.

For Open Graph or Twitter card meta tags, add them as plain `head.meta[]` entries. Baseline doesn't render OG or Twitter shells automatically yet.
