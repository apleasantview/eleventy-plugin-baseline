...
Baseline resolves the canonical URL in a fallback chain. It first looks at front matter `head.link[rel=canonical]`.

If absent, it computes one from `page.url`, `settings.url`, and `pathPrefix`. If `settings.url` is missing, it writes nothing.

Supplying `canonical` in front matter will not work Baseline computes it for you.
