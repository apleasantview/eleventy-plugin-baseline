---
title: 'Feature guides'
slug: 'feature-guide'
description: ''
date: 2026-05-03
permalink: '/docs/feature-guide/'
---

These feature tutorials build on each other. Starting from the simple site; you'll fill it in with assets, images, and head extras, then publish it through sitemaps and deployment. Debugging comes last, as the tool you reach for when something is off.

New to Baseline? The [overview](/docs/introduction/overview/) covers what it is and who it is for.

{% alertBlock "info" %}

The completed tutorial site (with how-to chapters) is [available on GitHub](https://github.com/apleasantview/simple-baseline-site). [Note: Currently under revision]

{% endalertBlock %}

## Index

<ul>
{%- for item in collections.docs %}
{%- if item.data.topicSlug === topicSlug and item.url !== page.url %}
	<li><a href="{{ item.url }}">{{ item.data.title }}</a></li>
{%- endif %}
{%- endfor %}
</ul>
