import { describe, it, expect } from 'vitest';
import translationIn from '../filters/translation-in.js';

const collection = [
	{ url: '/en/about/', translationKey: 'about', lang: 'en', isDefaultLang: true },
	{ url: '/nl/about/', translationKey: 'about', lang: 'nl', isDefaultLang: false },
	{ url: '/fr/about/', translationKey: 'about', lang: 'fr', isDefaultLang: false },
	{ url: '/en/orphan/' }
];

describe('translationIn', () => {
	it('returns the entry matching translationKey and lang', () => {
		const page = { translationKey: 'about', lang: 'en', isDefaultLang: true };
		const result = translationIn(page, collection, 'nl');
		expect(result).toEqual({
			url: '/nl/about/',
			translationKey: 'about',
			lang: 'nl',
			isDefaultLang: false
		});
	});

	it('returns null when the page has no translationKey', () => {
		const page = { lang: 'en', isDefaultLang: true };
		expect(translationIn(page, collection, 'nl')).toBeNull();
	});

	it('returns null when no entry matches the requested lang', () => {
		const page = { translationKey: 'about', lang: 'en', isDefaultLang: true };
		expect(translationIn(page, collection, 'de')).toBeNull();
	});

	it('returns null when no entry matches the translationKey', () => {
		const page = { translationKey: 'contact', lang: 'en', isDefaultLang: true };
		expect(translationIn(page, collection, 'nl')).toBeNull();
	});

	it('skips entries without a translationKey', () => {
		const page = { translationKey: 'about', lang: 'en', isDefaultLang: true };
		const result = translationIn(page, collection, 'fr');
		expect(result).not.toBeNull();
		expect(result.lang).toBe('fr');
	});
});
